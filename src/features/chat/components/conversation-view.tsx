...
export function ConversationView({
  ...
  const [selectedModel, setSelectedModel] = useState(
    initialSubmission?.model ?? DEFAULT_MODEL,
  )
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
        ...
        <div className="border-t border-[var(--line)] bg-[var(--surface-strong)] p-4">
          <ConversationComposer
                      ...
                      <div className="flex max-w-sm flex-col items-center gap-5 text-center">
                        ...
                        <div className="space-y-2">
                          ...
                          <p className="text-sm leading-6 text-[var(--sea-ink-soft)]">
                            Voice activity detected. Finish the take, then confirm
                            to send it for transcription.
                          </p>
                        </div>
                      </div>
                    ) : (
            ...
            statusHint={isBusy ? 'Streaming response.' : null}
            value={input}
          />
        </div>
      </div>
